...
#pragma message(                                                               \
    "opencl.hpp: CL_HPP_TARGET_OPENCL_VERSION is not defined. It will default to 310 (OpenCL 3.1)")
#define CL_HPP_TARGET_OPENCL_VERSION 310
    ...
    CL_HPP_TARGET_OPENCL_VERSION != 300 && CL_HPP_TARGET_OPENCL_VERSION != 310
#pragma message(                                                               \
    "opencl.hpp: CL_HPP_TARGET_OPENCL_VERSION is not a valid value (100, 110, 120, 200, 210, 220, 300 or 310). It will be set to 310 (OpenCL 3.1).")
...
#pragma message(                                                               \
    "opencl.hpp: CL_HPP_MINIMUM_OPENCL_VERSION is not a valid value (100, 110, 120, 200, 210, 220, 300 or 310). It will be set to 100")
...
#define CL_HPP_INIT_CL_EXT_FCN_PTR_PLATFORM_(platform, name)                   \
  if (!pfn_##name) {                                                           \
    pfn_##name = (PFN_##name)CL_(clGetExtensionFunctionAddressForPlatform)(    \
        platform, #name);                                                      \
  }
...
#define __GET_KERNEL_SUGGESTED_LWS_ERR                                         \
...
#define __SET_CONTEXT_DESTRUCTOR_CALLBACK_ERR                                  \
...
#define __CREATE_GL_RENDER_BUFFER_ERR                                          \
  CL_HPP_ERR_STR_(clCreateFromGLRenderbuffer)
...
#define CL_HPP_PARAM_NAME_CL_KHR_EXTERNAL_SEMAPHORE_DX_FENCE_EXT(F)            \
  F(cl_external_semaphore_handle_type_khr,                                     \
    CL_SEMAPHORE_HANDLE_D3D12_FENCE_KHR, void *)

#define CL_HPP_PARAM_NAME_CL_KHR_EXTERNAL_SEMAPHORE_OPAQUE_FD_EXT(F)           \
  F(cl_external_semaphore_handle_type_khr, CL_SEMAPHORE_HANDLE_OPAQUE_FD_KHR,  \
    int)
...
#define CL_HPP_PARAM_NAME_CL_IMAGE_REQUIREMENTS_EXT(F)                         \
  ...
  F(cl_image_requirements_info_ext, CL_IMAGE_REQUIREMENTS_MAX_DEPTH_EXT,       \
    cl_uint)                                                                   \
  F(cl_image_requirements_info_ext, CL_IMAGE_REQUIREMENTS_MAX_ARRAY_SIZE_EXT,  \
    cl_uint)
...
CL_HPP_PARAM_NAME_CL_KHR_EXTERNAL_SEMAPHORE_DX_FENCE_EXT(
    CL_HPP_DECLARE_PARAM_TRAITS_)
...
CL_HPP_DECLARE_PARAM_TRAITS_(cl_device_info,
                             ...
                             cl::vector<const char *>)
CL_HPP_DECLARE_PARAM_TRAITS_(cl_device_info, CL_DEVICE_SPIRV_EXTENSIONS_KHR,
                             cl::vector<const char *>)
CL_HPP_DECLARE_PARAM_TRAITS_(cl_device_info, CL_DEVICE_SPIRV_CAPABILITIES_KHR,
                             cl::vector<cl_uint>)
...
template <> struct ReferenceHandler<cl_mem> {
  static cl_int retain(cl_mem memory) { return CL_(clRetainMemObject)(memory); }
...
template <> struct ReferenceHandler<cl_kernel> {
  static cl_int retain(cl_kernel kernel) { return CL_(clRetainKernel)(kernel); }
...
static cl_uint getDevicePlatformVersion(cl_device_id device) {
  ...
  CL_(clGetDeviceInfo)(device, CL_DEVICE_PLATFORM, sizeof(platform), &platform,
                       nullptr);
...
public:
   ...
   * OpenCL device. If \a devices argument is nullptr, this argument is ignored.
   *
  ...
  cl_int getDevices(cl_d3d10_device_source_khr d3d_device_source,
    ...
    vector<cl_device_id> ids(n);
    err =
        pfn_clGetDeviceIDsFromD3D10KHR(object_, d3d_device_source, d3d_object,
                                       d3d_device_set, n, ids.data(), nullptr);
    if (err != CL_SUCCESS) {
      return detail::errHandler(err, __GET_DEVICE_IDS_ERR);
    }
...
private:
  ...
  static void makeDefault() {
...
#endif
    {
      ...
      default_ = Context(CL_DEVICE_TYPE_DEFAULT, properties, nullptr, nullptr,
                         &default_error_);
    }
#if defined(CL_HPP_ENABLE_EXCEPTIONS)
    catch (cl::Error &e) {
...
public:
  ...
  Context(const vector<Device> &devices,
    ...
    object_ =
        CL_(clCreateContext)(properties, (cl_uint)deviceIDs.size(),
  ...
  Context(const Device &device,
    ...
    object_ = CL_(clCreateContext)(properties, 1, &deviceID, notifyFptr, data,
                                   &error);
...
#endif
    object_ = CL_(clCreateContextFromType)(properties, type, notifyFptr, data,
                                           &error);
  ...
  cl_int getSupportedImageFormats(cl_mem_flags flags, cl_mem_object_type type,
    ...
    if (numEntries > 0) {
      ...
      err = CL_(clGetSupportedImageFormats)(object_, flags, type, numEntries,
  ...
  template <typename T>
  cl_int
  getImageRequirementsInfoExt(cl_image_requirements_info_ext name, T *param,
                              cl_mem_flags flags = 0,
  ...
  getImageRequirementsInfoExt(cl_mem_flags flags = 0,
    ...
    cl_int result = getImageRequirementsInfoExt(type, &param, flags, properties,
...
public:
  ...
  pointer allocate(size_type size,
    ...
    void *voidPointer = CL_(clSVMAlloc)(context_(), SVMTrait::getSVMMemFlags(),
                                        size * sizeof(T), 0);
...
public:
  ...
  Buffer(const Context &context, const vector<cl_mem_properties> &properties,
    ...
    object_ = CL_(clCreateBufferWithProperties)(
        context(), properties.empty() ? nullptr : properties.data(), flags,
        size, host_ptr, &error);
  ...
  Buffer createSubBuffer(cl_mem_flags flags,
    ...
    Buffer result;
    cl_int error;
    result.object_ = CL_(clCreateSubBuffer)(object_, flags, buffer_create_type,
...
public:
  ...
  Image1D(const Context &context, cl_mem_flags flags, ImageFormat format,
    ...
    object_ =
        CL_(clCreateImage)(context(), flags, &format, &desc, host_ptr, &error);
...
public:
  Image1DBuffer(const Context &context, cl_mem_flags flags, ImageFormat format,
                size_type width, const Buffer &buffer, cl_int *err = nullptr) {
    cl_int error;
    ...
    object_ =
        CL_(clCreateImage)(context(), flags, &format, &desc, nullptr, &error);
  ...
  Image1DBuffer(const Context &context,
                const vector<cl_mem_properties> &properties, cl_mem_flags flags,
...
public:
  ...
  Image1DArray(const Context &context,
               const vector<cl_mem_properties> &properties, cl_mem_flags flags,
               ImageFormat format, size_type arraySize, size_type width,
               size_type rowPitch = 0, void *host_ptr = nullptr,
               cl_int *err = nullptr) {
    cl_int error;
...
#if CL_HPP_MINIMUM_OPENCL_VERSION < 120
    if (!useCreateImage) {
      object_ = CL_(clCreateImage2D)(context(), flags, &format, width, height,
...
public:
  ...
  Image2DGL(const Context &context, cl_mem_flags flags, cl_GLenum target,
    ...
    cl_int error;
    object_ = CL_(clCreateFromGLTexture2D)(context(), flags, target, miplevel,
                                           texobj, &error);
...
public:
  ...
  Image2DArray(const Context &context,
               const vector<cl_mem_properties> &properties, cl_mem_flags flags,
               ImageFormat format, size_type arraySize, size_type width,
               size_type height, size_type rowPitch = 0,
...
public:
  ...
  Image3D(const Context &context, cl_mem_flags flags, ImageFormat format,
    ...
    if (!useCreateImage) {
      object_ =
          CL_(clCreateImage3D)(context(), flags, &format, width, height, depth,
...
public:
  ...
  Image3DGL(const Context &context, cl_mem_flags flags, cl_GLenum target,
            cl_GLint miplevel, cl_GLuint texobj, cl_int *err = nullptr) {
    cl_int error;
    object_ = CL_(clCreateFromGLTexture3D)(context(), flags, target, miplevel,
                                           texobj, &error);
...
public:
  ...
  Sampler(const Context &context, cl_bool normalized_coords,
    ...
    cl_sampler_properties sampler_properties[] = {CL_SAMPLER_NORMALIZED_COORDS,
                                                  ...
                                                  filter_mode,
                                                  0};
    object_ = CL_(clCreateSamplerWithProperties)(context(), sampler_properties,
                                                 &error);
...
public:
  ...
  cl_int getArgInfo(cl_uint argIndex, cl_kernel_arg_info name, T *param) const {
    return detail::errHandler(detail::getInfo(CL_(clGetKernelArgInfo), object_,
                                              argIndex, name, param),
  ...
  cl_int setArg(cl_uint index, size_type size, const void *argPtr) {
    return detail::errHandler(CL_(clSetKernelArg)(object_, index, size, argPtr),
                              __SET_KERNEL_ARGS_ERR);
  }
...
public:
  ...
  Program(const string &source, bool build = false, cl_int *err = nullptr) {
    ...
    object_ = CL_(clCreateProgramWithSource)(context(), (cl_uint)1, &strings,
                                             &length, &error);
  ...
  Program(const Sources &sources, cl_int *err = nullptr) {
    ...
    object_ = CL_(clCreateProgramWithSource)(
        context(), (cl_uint)n, strings.empty() ? nullptr : strings.data(),
        lengths.empty() ? nullptr : lengths.data(), &error);
   ...
   * binary pointer is nullptr. Set to CL_INVALID_BINARY if the binary provided
   * is not valid for the matching device.
   * \param err if non-nullptr will be set to CL_SUCCESS on successful operation
   * or one of the following errors: CL_INVALID_CONTEXT if context is not a
   * valid context. CL_INVALID_VALUE if the length of devices is zero; or if the
   * length of binaries does not match the length of devices; or if any entry in
   * binaries is nullptr or has length 0. CL_INVALID_DEVICE if OpenCL devices
   * listed in devices are not in the list of devices associated with context.
  ...
  Program(const Context &context, const vector<Device> &devices,
    ...
    object_ = CL_(clCreateProgramWithBinary)(
        context(), (cl_uint)deviceIDs.size(),
        deviceIDs.empty() ? nullptr : deviceIDs.data(),
  ...
  Program(const Context &context, const vector<Device> &devices,
    ...
    object_ = CL_(clCreateProgramWithBuiltInKernels)(
        context(), (cl_uint)deviceIDs.size(),
        deviceIDs.empty() ? nullptr : deviceIDs.data(), kernelNames.c_str(),
        &error);
  ...
  cl_int build(const vector<Device> &devices, const char *options = nullptr,
    ...
    cl_int buildError =
        CL_(clBuildProgram)(object_, (cl_uint)deviceIDs.size(),
                            deviceIDs.empty() ? nullptr : deviceIDs.data(),
  ...
  cl_int build(const Device &device, const char *options = nullptr,
    ...
    cl_int buildError =
        CL_(clBuildProgram)(object_, 1, &deviceID, options, notifyFptr, data);
  ...
  cl_int compile(const char *options = nullptr,
                 void(CL_CALLBACK *notifyFptr)(cl_program, void *) = nullptr,
                 void *data = nullptr) const {
    cl_int error = CL_(clCompileProgram)(object_, 0, nullptr, options, 0,
                                         nullptr, nullptr, notifyFptr, data);
  ...
  cl_int compile(const char *options, const vector<Program> &inputHeaders,
    ...
    cl_int error = CL_(clCompileProgram)(
        object_, 0, nullptr, options, static_cast<cl_uint>(inputHeaders.size()),
        reinterpret_cast<const cl_program *>(
            inputHeaders.empty() ? nullptr : inputHeaders.data()),
        reinterpret_cast<const char **>(headerIncludeNamesCStr.empty()
                                            ? nullptr
  ...
  cl_int compile(const char *options, const vector<Device> &devices,
                 const vector<Program> &inputHeaders = vector<Program>(),
                 const vector<string> &headerIncludeNames = vector<string>(),
                 void(CL_CALLBACK *notifyFptr)(cl_program, void *) = nullptr,
                 void *data = nullptr) const {
    ...
    cl_int error = CL_(clCompileProgram)(
        object_, static_cast<cl_uint>(deviceIDs.size()),
        reinterpret_cast<const cl_device_id *>(
            deviceIDs.empty() ? nullptr : deviceIDs.data()),
        options, static_cast<cl_uint>(inputHeaders.size()),
        reinterpret_cast<const cl_program *>(
            inputHeaders.empty() ? nullptr : inputHeaders.data()),
        reinterpret_cast<const char **>(headerIncludeNamesCStr.empty()
                                            ? nullptr
  ...
  cl_int createKernels(vector<Kernel> *kernels) {
    ...
    err = CL_(clCreateKernelsInProgram)(
        object_, numKernels, value.empty() ? nullptr : value.data(), nullptr);
    if (err != CL_SUCCESS) {
      return detail::errHandler(err, __CREATE_KERNELS_IN_PROGRAM_ERR);
    }
...
inline Program linkProgram(const Program &input1, const Program &input2,
  ...
  cl_program prog = CL_(clLinkProgram)(ctx(), 0, nullptr, options, 2, programs,
...
linkProgram(const vector<Program> &inputPrograms, const char *options = nullptr,
  ...
  cl_program prog = CL_(clLinkProgram)(
      ...
      reinterpret_cast<const cl_program *>(
          inputPrograms.empty() ? nullptr : inputPrograms.data()),
      notifyFptr, data, &error_local);
...
public:
  ...
  CommandQueue(cl_command_queue_properties properties, cl_int *err = nullptr) {
    ...
    if (error != CL_SUCCESS) {
      ...
      if (!useWithProperties) {
        object_ =
            CL_(clCreateCommandQueue)(context(), device(), properties, &error);
...
#if CL_HPP_MINIMUM_OPENCL_VERSION < 200
    if (!useWithProperties) {
      object_ = CL_(clCreateCommandQueue)(context(), devices[0](), properties,
                                          &error);
  ...
  cl_int enqueueReadBuffer(const Buffer &buffer, cl_bool blocking,
    ...
    cl_int err = detail::errHandler(
        CL_(clEnqueueReadBuffer)(object_, buffer(), blocking, offset, size, ptr,
  ...
  cl_int enqueueCopyBufferRect(
    ...
    cl_int err = detail::errHandler(
        CL_(clEnqueueCopyBufferRect)(
            ...
            dst_slice_pitch, (events != nullptr) ? (cl_uint)events->size() : 0,
            (events != nullptr && events->size() > 0)
                ? (const cl_event *)&events->front()
                : nullptr,
            (event != nullptr) ? &tmp : nullptr),
  ...
  cl_int enqueueReadImage(const Image &image, cl_bool blocking,
    ...
    cl_int err = detail::errHandler(
        CL_(clEnqueueReadImage)(
            ...
            slice_pitch, ptr, (events != nullptr) ? (cl_uint)events->size() : 0,
            (events != nullptr && events->size() > 0)
                ? (const cl_event *)&events->front()
                : nullptr,
            (event != nullptr) ? &tmp : nullptr),
  ...
  cl_int enqueueCopyImage(const Image &src, const Image &dst,
    ...
    cl_int err = detail::errHandler(
        CL_(clEnqueueCopyImage)(
            object_, src(), dst(), src_origin.data(), dst_origin.data(),
            region.data(), (events != nullptr) ? (cl_uint)events->size() : 0,
            (events != nullptr && events->size() > 0)
                ? (const cl_event *)&events->front()
                : nullptr,
            (event != nullptr) ? &tmp : nullptr),
  ...
  void *enqueueMapImage(const Image &image, cl_bool blocking,
                        ...
                        const array<size_type, 2> &region, size_type *row_pitch,
                        size_type *slice_pitch,
                        const vector<Event> *events = nullptr,
                        Event *event = nullptr, cl_int *err = nullptr) const {
    return enqueueMapImage(image, blocking, flags, {origin[0], origin[1], 0},
                           {region[0], region[1], 1}, row_pitch, slice_pitch,
                           events, event, err);
  }
   ...
   * to the destination pointer. This function is specifically for transferring
   * data between the host and a coarse-grained SVM buffer.
   */
  template <typename T>
  cl_int enqueueMemcpySVM(T *dst_ptr, const T *src_ptr, cl_bool blocking,
    ...
    cl_event tmp;
    cl_int err = detail::errHandler(
        CL_(clEnqueueSVMMemcpy)(object_, blocking, static_cast<void *>(dst_ptr),
  ...
  /**
   *Enqueues a command that will copy data from one coarse-grained SVM buffer to
   * another. This function takes two cl::pointer instances representing the
  ...
  /**
   * Enqueues a command that fills a region of a coarse-grained SVM buffer with
   * a specified pattern. This variant takes a cl::pointer instance.
   */
  ...
  cl_int enqueueMemFillSVM(cl::vector<T, Alloc> &container, PatternType pattern,
                           const vector<Event> *events = nullptr,
                           Event *event = nullptr) const {
    cl_event tmp;
    cl_int err = detail::errHandler(
        CL_(clEnqueueSVMMemFill)(
            object_, container.empty() ? nullptr : container.data(), &pattern,
  ...
  cl_int enqueueMapSVM(T *ptr, cl_bool blocking, cl_map_flags flags,
    ...
    cl_event tmp;
    cl_int err = detail::errHandler(
        CL_(clEnqueueSVMMap)(object_, blocking, flags, static_cast<void *>(ptr),
                             size,
                             (events != nullptr) ? (cl_uint)events->size() : 0,
                             (events != nullptr && events->size() > 0)
                                 ? (const cl_event *)&events->front()
  ...
  cl_int enqueueMapSVM(cl::pointer<T, D> &ptr, cl_bool blocking,
    ...
    cl_event tmp;
    cl_int err = detail::errHandler(
        CL_(clEnqueueSVMMap)(object_, blocking, flags,
  ...
  cl_int enqueueUnmapSVM(cl::vector<T, Alloc> &container,
    ...
    cl_int err = detail::errHandler(
        CL_(clEnqueueSVMUnmap)(
            object_, container.empty() ? nullptr : container.data(),
            (events != nullptr) ? (cl_uint)events->size() : 0,
            (events != nullptr && events->size() > 0)
                ? (const cl_event *)&events->front()
                : nullptr,
  ...
  cl_int enqueueMigrateMemObjects(const vector<Memory> &memObjects,
    ...
    cl_int err = detail::errHandler(
        CL_(clEnqueueMigrateMemObjects)(
            ...
            localMemObjects.empty() ? nullptr : localMemObjects.data(), flags,
            (events != nullptr) ? (cl_uint)events->size() : 0,
            (events != nullptr && events->size() > 0)
                ? (const cl_event *)&events->front()
                : nullptr,
  ...
  cl_int enqueueMigrateSVM(const cl::vector<T *> &svmRawPointers,
    ...
    cl_int err = detail::errHandler(
        CL_(clEnqueueSVMMigrateMem)(
            ...
            sizes.empty() ? nullptr : sizes.data(), flags,
            (events != nullptr) ? (cl_uint)events->size() : 0,
            (events != nullptr && events->size() > 0)
                ? (const cl_event *)&events->front()
                : nullptr,
 ...
 */
  ...
  cl_int enqueueAcquireD3D10Objects(const vector<Memory> *mem_objects = nullptr,
                                    const vector<Event> *events = nullptr,
                                    Event *event = nullptr) const {
  ...
  cl_int enqueueReleaseD3D10Objects(const vector<Memory> *mem_objects = nullptr,
                                    const vector<Event> *events = nullptr,
                                    Event *event = nullptr) const {
 ...
 */
    ...
    if (pfn_clEnqueueAcquireExternalMemObjectsKHR) {
      err = pfn_clEnqueueAcquireExternalMemObjectsKHR(
          ...
          reinterpret_cast<const cl_mem *>(
              mem_objects.empty() ? nullptr : mem_objects.data()),
          (events_wait != nullptr) ? static_cast<cl_uint>(events_wait->size())
                                   : 0,
  ...
  cl_int enqueueSignalSemaphores(
      const vector<Semaphore> &sema_objects,
      const vector<cl_semaphore_payload_khr> &sema_payloads = {},
      const vector<Event> *events_wait_list = nullptr, Event *event = nullptr);
#endif // cl_khr_semaphore

#if CL_HPP_TARGET_OPENCL_VERSION >= 310
  NDRange getKernelSuggestedLocalWorkSize(const Kernel &kernel,
    ...
    cl_int error = detail::errHandler(
        CL_(clGetKernelSuggestedLocalWorkSize)(
            object_, kernel(), (cl_uint)global.dimensions(),
            offset.dimensions() != 0 ? offset.get() : nullptr, global.get(),
            local.get()),
...
public:
  ...
  DeviceCommandQueue(DeviceQueueProperties properties, cl_int *err = nullptr) {
    ...
    cl_queue_properties queue_properties[] = {CL_QUEUE_PROPERTIES,
                                              mergedProperties, 0};
    object_ = CL_(clCreateCommandQueueWithProperties)(context(), device(),
...
inline void *enqueueMapBuffer(const Buffer &buffer, cl_bool blocking,
  ...
  void *result =
      CL_(clEnqueueMapBuffer)(queue(), buffer(), blocking, flags, offset, size,
                              (events != nullptr) ? (cl_uint)events->size() : 0,
                              (events != nullptr && events->size() > 0)
                                  ? (const cl_event *)&events->front()
                                  : nullptr,
...
inline cl_int enqueueUnmapMemObject(const Memory &memory, void *mapped_ptr,
  ...
  cl_event tmp;
  cl_int err =
      detail::errHandler(CL_(clEnqueueUnmapMemObject)(
...
inline cl_int enqueueCopyImageToBuffer(const Image &src, const Buffer &dst,
                                       ...
                                       Event *event = nullptr) {
  return enqueueCopyImageToBuffer(src, dst, {src_origin[0], src_origin[1], 0},
                                  {region[0], region[1], 1}, dst_offset, events,
                                  event);
}
...
public:
  ...
  Semaphore(const Context &context,
    ...
    if (pfn_clCreateSemaphoreWithPropertiesKHR) {
      object_ = pfn_clCreateSemaphoreWithPropertiesKHR(
          context(), sema_props.empty() ? nullptr : sema_props.data(), &error);
    }
  ...
  cl_int getHandleForTypeKHR(const Device &device,
    ...
    return detail::errHandler(
        detail::getInfo(pfn_clGetSemaphoreHandleForTypeKHR, object_, device(),
                        name, param),
        __GET_SEMAPHORE_HANDLE_FOR_TYPE_KHR_ERR);
  }
...
public:
  ...
  explicit CommandBufferKhr(const vector<CommandQueue> &queues,
    ...
    if (pfn_clCreateCommandBufferKHR) {
      object_ = pfn_clCreateCommandBufferKHR(
          (cl_uint)queues.size(), (const cl_command_queue *)&queues.front(),
  ...
  cl_int commandBarrierWithWaitList(
    ...
    cl_int error = detail::errHandler(
        pfn_clCommandBarrierWithWaitListKHR(
            object_, (command_queue != nullptr) ? (*command_queue)() : nullptr,
  ...
  cl_int
  commandCopyBuffer(const Buffer &src, const Buffer &dst, size_type src_offset,
  ...
  cl_int commandCopyBufferRect(
    ...
    cl_int error = detail::errHandler(
        pfn_clCommandCopyBufferRectKHR(
            ...
            nullptr, // Properties
#endif
            src(), dst(), src_origin.data(), dst_origin.data(), region.data(),
  ...
  cl_int commandCopyImage(
      const Image &src, const Image &dst, const array<size_type, 3> &src_origin,
      const array<size_type, 3> &dst_origin, const array<size_type, 3> &region,
      const vector<cl_sync_point_khr> *sync_points_vec = nullptr,
      cl_sync_point_khr *sync_point = nullptr,
      MutableCommandKhr *mutable_handle = nullptr,
  ...
  template <typename PatternType>
  cl_int
  commandFillBuffer(const Buffer &buffer, PatternType pattern, size_type offset,
                    size_type size,
                    const vector<cl_sync_point_khr> *sync_points_vec = nullptr,
                    cl_sync_point_khr *sync_point = nullptr,
                    MutableCommandKhr *mutable_handle = nullptr,
  ...
  commandFillImage(const Image &image, cl_float4 fillColor,
    ...
    cl_int error = detail::errHandler(
        pfn_clCommandFillImageKHR(
            ...
            nullptr, // Properties
#endif
            image(), &fillColor, origin.data(), region.data(),
            (sync_points_vec != nullptr) ? (cl_uint)sync_points_vec->size() : 0,
            (sync_points_vec != nullptr && sync_points_vec->size() > 0)
                ? &sync_points_vec->front()
                : nullptr,
  ...
  cl_int commandNDRangeKernel(
    ...
    cl_int error = detail::errHandler(
        pfn_clCommandNDRangeKernelKHR(
            object_, (command_queue != nullptr) ? (*command_queue)() : nullptr,
            properties.empty() ? nullptr : properties.data(), kernel(),
  ...
  cl_int updateMutableCommands(
    ...
    return detail::errHandler(
        pfn_clUpdateMutableCommandsKHR(
            object_, static_cast<cl_uint>(configs.size()),
